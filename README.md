...
The RSEM .genes.results files from the previous step are loaded into R and merged into a single count matrix. Sample metadata (such as culture condition, cell line) is read from a separate file and used to set up a paired experimental design.
